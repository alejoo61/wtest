...
const MyWilderslFNFTDetails = (props) => {
  const {
    tokenId,
    tokenName,
    mintedBy,
  ...
  const callGift = (tokenId, receiver) => {
    props.giftWilderslF(tokenId, receiver);
  };

  return (
    <div key={tokenId.toNumber()} className="mt-4 text-white">
      <p className="text-center">
        <span className="font-weight-bold text-center">Token Id</span> :{" "}
        {tokenId.toNumber()}
      </p>
      <p className="text-center">
        <span className="font-weight-bold text-center">Name</span> : {tokenName}
      </p>
      <p className="text-center">
        <span className="font-weight-bold text-center">No. of Transfers</span> :{" "}
      ...
      props.accountAddress !== previousOwner ? (
        ...
        <form
              onSubmit={(e) => {
                e.preventDefault();
                callGift(
                  props.WilderslF.tokenId.toNumber(),
                  state.receiver
                );
              }}
            >
...
export default MyWilderslFNFTDetails;
